...
describe('管理页卡片顺序与默认展开状态', () => {
  it('发布公告排最上面，账号设置和新增成员保持可见', () => {
    ...
    expect(manage).toContain('<details id="account-settings" className="workspaceCard manageAccountCard collapsibleToolCard" open>');
    expect(side).toContain('<details className="workspaceCard addMemberCard collapsibleToolCard" open>');
  });
