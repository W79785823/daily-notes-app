...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  let team;
  try {
    team = await prisma.team.update({ where: { id }, data: { active: true } });
  } catch (error) {
  ...
  await prisma.auditLog.create({ data: { teamId: team.id, action: 'admin.team.reactivate', userId: admin.id, detail: { teamId: team.id, teamName: team.name } } });
  if (isForm) redirectWithParam('/admin', 'ok', 'team.reactivated');
